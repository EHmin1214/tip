...
- 전극: MIDA v1.0 두피에 표준 10-10, `Create1010System` → `PlaceElectrodes`
  - **기본 세트는 3 cm²**(r=9.77mm, h=2mm) — `leadfield_rebuild_3cm2`, 84전극
  - 옛 세트 `leadfield_rebuild` 는 0.5 cm²(r=4mm). `TIP_LEADFIELD_DIR` 로 되돌릴 수 있다
- 솔브 시간: 전극당 약 2.2~2.5분 (3 cm² 는 Dirichlet 점이 늘어 조금 느리다)
- ⚠ **"전극 크기는 결과에 무관" 은 2026-08-14 실측으로 반증됐다.** 옛 근거였던
  "3cm² vs 0.5cm² 99.8% 동일" 은 이전 머신 산물이라 이 머신에서 재검증할 수 없었고,
  84전극을 3 cm² 로 다시 풀어 공개 CSV(4표적 53몽타주)로 재니:

  | | 0.5 cm² | **3 cm²** |
  |---|---:|---:|
  | M1비 (우리÷공개값) | 1.139 | **1.118** |
  | M2비 | 0.956 | **0.988** |
  | M3비 | 1.098 | **1.053** |
  | 몽타주 선정 후회(WP 손실) | 29.4% | **10.1%** |
  | 1 V 주입전류 | 1.07~1.24 mA | **1.84~2.53 mA** |

  실제 프로토콜·tip.lite·공개값이 전부 3 cm² 이므로 **물리적으로도 3 cm² 가 맞다.**
  단 **최적 몽타주는 안 바뀐다**(좌시상 27전극 68만 조합에서 동일). 상세는
  [[seqti-leadfield-rebuild]] 메모리와 `outputs/research/metrics_by_region.md`.
